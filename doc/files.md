...
The data directory is the default location where the MENA Bitcoin Core files are stored.
...
macOS    | `$HOME/Library/Application Support/MENA Bitcoin/`
Windows  | `%APPDATA%\MENA Bitcoin\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `menabitcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `menabitcoin.conf`        | Contains [configuration settings](bitcoin-conf.md) for `menabitcoind` or `menabitcoin-qt`; can be specified by `-conf` option
`./`               | `menabitcoind.pid`        | Stores the process ID (PID) of `menabitcoind` or `menabitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `menabitcoind` or `menabitcoin-qt`; can be specified by `-debuglogfile` option
...
`menabitcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
