...
UniValue CreateColdStakeDelegation(const UniValue& params, CWallet* const pwallet)
{
    ...
    if (pwallet->GetLastBlockHeight() < Params().GetConsensus().MENAColdStakeEnableHeight) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Cold Staking is enabled at block %d\n", Params().GetConsensus().MENAColdStakeEnableHeight));
    }
    ...
    if (!IsValidDestination(stakeAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MENA Bitcoin address");
    ...
    if (params.size() > 1 && !params[1].isNull() && !params[1].get_str().empty()) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MENA Bitcoin spending address.");
...
static RPCHelpMan delegatestake()
{
    return RPCHelpMan{"delegatestake",
                ...
                HELP_REQUIRING_PASSPHRASE,
                {
                    {"staker_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The MENA Bitcoin address to delegate to."},
                    {"owner_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The MENA Bitcoin address corresponding to the key that will be able to spend the stake."},
