...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (forDelegation && model->wallet().wallet()->GetLastBlockHeight() < Params().GetConsensus().MENAColdStakeEnableHeight)
    {
        Q_EMIT message(tr("Cold Staking not enabled"), tr("Cold staking is enabled at block %1").arg(Params().GetConsensus().MENAColdStakeEnableHeight), CClientUIInterface::MSG_ERROR);
        return;
    }
